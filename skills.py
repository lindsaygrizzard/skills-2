...
def count_unique(string1):
    ...
    wordsl = string1.split( )

       
        
    counts = dict()
    for word in wordsl:
        counts[word] = counts.get(word, 0) + 1 #key = value (get value of key or return 0) 
                                                    # + 1 for each time found
...
def sum_zero(list1):
    ...
    for itema in list1:
        for itemb in list1:
            if itema + itemb == 0 and (itemb, itema) not in make_zero_pairs:
                make_zero_pairs.add((itema, itemb))
    return make_zero_pairs
...
def find_duplicates(words):
    ...
    """
    I_dont_have_time_for_no_dups = list(set(words))
    return I_dont_have_time_for_no_dups
...
def word_length(words):
    ...
    for vocable in words:
        le_dictionaire.setdefault(len(vocable), []).append(vocable) #key = length of word (ex 2)..
                                                                    #append word to that key
    return sorted(le_dictionaire.items())  #sort by key will put in numerical order
